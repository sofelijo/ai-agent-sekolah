...
def normalize_input(text):
    ...
    replacements = {
        "umur pendaftar": "umur", "usia pendaftar": "umur", "usia siswa": "umur",
        "umur siswa": "umur", "pendaftar termuda": "umur terendah", "pendaftar tertua": "umur tertinggi",
        "usia paling muda": "umur terendah", "usia paling tua": "umur tertinggi", "ranking": "urutan",
        "anbk untuk sd kapan": "anbk untuk sd jadwalnya kapan", "kapan anbk sd": "jadwal anbk sd",
        "anbk sd kapan": "jadwal anbk sd", "jadwal anbk": "jadwal anbk sd",
    }
...
def coerce_to_text(result_obj):
    ...
    if isinstance(result_obj, str): return result_obj
    if hasattr(result_obj, "content"):
        if isinstance(result_obj.content, str): return result_obj.content
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        history_from_db = get_chat_history(user_id, limit=5) # Ambil lebih banyak history
        ...
        if not response.strip():
            response = ("😅 Maaf nih, *ASKA* belum nemu jawabannya di data sekolah. "
